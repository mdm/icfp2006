...
DV       j = XLVIII
DX       k = XLVIII
DXV      IF CHECKPASS(username, words(i) + CHR(j) + CHR(k)) THEN GOTO DLXV
DXX	 k = k + I
DXXV	 IF k < LVIII THEN GOTO DXV
DXXX	 j = j + I
DXXXV	 IF j < LVIII THEN GOTO DX
DXL	 i = i + I
DXLV	 IF i > pwdcount THEN GOTO DLV
DL	 GOTO DV
DLV      PRINT "no matches for user " + username + CHR(X)
DLX	 END
DLXV     PRINT "found match!! for user " + username + CHR(X)
DLXX     PRINT "password: " + words(i) + CHR(j) + CHR(k) + CHR(X)
DLXXV    END
STOP
